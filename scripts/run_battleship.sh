...
fairseq-train data/BinFairseqLibriUnits \
   ...
   --arch iwslt_wordlen_transformer --optimizer adam \
    ...
    --max-epoch 200 \
    --maximize-best-checkpoibnt-metric --best-checkpoint-metric wer bleu --eval-bleu --eval-bleu-args '{"beam": 5, "max_len_a": 1.2, "max_len_b": 10}' --eval-bleu-detok moses --eval-bleu-remove-bpe --eval-bleu-print-samples
    # ` # evaluation ` \
    # --eval-bleu \
